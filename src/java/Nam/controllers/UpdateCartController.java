...
//            for (BookDTO dto : cart.getCart().values()) {
            ...
            for (BookDTO dto : booklist) {
                if (dto.getBookID().equals(id)) {
                    ...
                    String image = dto.getBookImage();
                    int CategoryID = dto.getCategoryID();
                    String Status = dto.getStatus();
                    if (dto.getQuantity() >= newquantity) {
                        book = new BookDTO(id, name, price, image, newquantity, CategoryID, Status);
                        cart.update(id, book);
                        session.setAttribute("CART", cart);
                        url = SUCCESS;
                        break;
                    } else {
                        String error_msg = "Error: Quantity of " + name + " book Is larger the avalable Quantity";
